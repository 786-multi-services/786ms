...
class UserView(CreateView):
	template_name = 'career/trainer_career.html'
	model=User
	form_class = UserForm
	success_url = 'submit/'
...
def index(request):
	return render(request,'career/index.html',{'active_nav_link':'career',})

def trainer_view(request,token):
	try:
		token=Token.objects.get(token=token)
		user=User.objects.get(token=token)
		return render(request,'career/trainer_submit_career.html',{
